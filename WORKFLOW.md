...
- [x] `src/tokens/` 디렉토리 생성
- [x] `src/tokens/colors.css` — 시맨틱 색상 변수 정의
...
- [x] before의 하드코딩 색상값 목록 정리 후 토큰으로 매핑
...
- [x] `npx shadcn-ui@latest add button` 또는 직접 구현
- [x] CVA로 `variant` (default, primary, secondary, destructive, outline, ghost) 및 `size` (sm, md, lg) 정의
- [x] 도메인 props(`entityType`, `action`, `entity`) 완전 제거 — 순수 UI 컴포넌트로
- [x] Storybook story 작성 (`src/stories/Button.stories.tsx`)
...
- [x] `npx shadcn-ui@latest add badge` 또는 직접 구현
- [x] CVA로 `variant` (default, secondary, destructive, outline, success, warning) 정의
- [x] 도메인 props 완전 제거 — `children`과 `variant`만으로 표현
- [x] 도메인별 매핑은 사용하는 쪽(Table, Page)에서 처리하도록 이관
- [x] Storybook story 작성
